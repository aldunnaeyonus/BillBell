...
export default function BulkImport() {
  ...
  function parseCsvToBills(csv: string): any[] {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (!cleanDueDate) continue; 
      const cleanAmountStr = amountStr.replace(/[^0-9.-]+/g, "");
      const amount = parseFloat(cleanAmountStr);
      if (Number.isNaN(amount) || amount <= 0) continue;

      result.push({ name, amount, due_date: cleanDueDate, notes, recurrence, offset, reminder, paymentMethod });
    }
    return result;
  }
  ...
  async function handleWebPortalPress() {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: t("Share Link"), onPress: () => { Share.open({ message: t("Access the BillBell Web Portal to upload your bills:"), url: url }).catch((err) => { if (err) console.log(err); }); } },
  ...
  async function downloadTemplate() {
    try {
      ...
      await Share.open({ url: templateFile.uri, type: "text/csv", filename: "bill_import_template", title: "Download Bill Template" });
    } catch (error: any) {
  ...
  async function doImport() {
    if (!importCode.trim()) { Alert.alert(t("Import"), t("Please enter an import code.")); return; }
    if (!bills.length) { Alert.alert(t("Import"), t("Please pick a CSV and parse some bills first.")); return; }
    try {
      setLoading(true);
      await api.importBills(importCode.trim().toUpperCase(), bills);
      if (isMounted.current) { Alert.alert(t("Import"), t("Bills imported successfully!")); setImportCode(""); setCsvName(null); setBills([]); router.back(); }
    } catch (e: any) {
        ...
        <View style={styles.content}>
          ...
          <View style={styles.section}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                styles.actionCard,
                { backgroundColor: theme.colors.card, borderColor: theme.colors.border, opacity: downloading || pressed ? 0.7 : 1 },
              ]}
            >
              <View style={[styles.iconBox, { backgroundColor: theme.mode === 'dark' ? '#1E293B' : '#F1F5F9' }]}>
                {downloading ? <ActivityIndicator size="small" color={theme.colors.primary} /> : <Ionicons name="download-outline" size={22} color={theme.colors.primary} />}
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.cardTitle, { color: theme.colors.primaryText }]}>{t("Download Template")}</Text>
                <Text style={[styles.cardSubtitle, { color: theme.colors.subtext }]}>{t("Get the CSV template to ensure correct formatting.")}</Text>
              </View>
          ...
          <View style={styles.section}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                styles.actionCard,
                { backgroundColor: theme.colors.card, borderColor: theme.colors.border, opacity: pressed ? 0.7 : 1, marginBottom: 8 },
              ]}
            >
              <View style={[styles.iconBox, { backgroundColor: theme.mode === 'dark' ? '#1E293B' : '#F1F5F9' }]}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.cardTitle, { color: theme.colors.primaryText }]}>{t("visit_on_computer")}</Text>
                <Text style={[styles.cardSubtitle, { color: theme.colors.subtext }]}>{t("visit_on_computer_subtitle")}</Text>
              </View>
          ...
          <Pressable
            ...
            style={({ pressed }) => [
              styles.importButton,
              { backgroundColor: theme.colors.primary, opacity: loading || !importCode.trim() || !bills.length ? 0.3 : pressed ? 0.8 : 1 },
            ]}
          >
            {loading ? <ActivityIndicator color={theme.colors.primaryTextButton} /> : <Text style={[styles.importButtonText, { color: theme.colors.primaryTextButton }]}>{t("Start Import")}</Text>}
          </Pressable>
        </View>
      </View>
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  actionCard: { flexDirection: "row", alignItems: "center", padding: 16, borderRadius: 16, borderWidth: 1, gap: 12 },
  iconBox: { width: 40, height: 40, borderRadius: 12, justifyContent: "center", alignItems: "center" },
  cardTitle: { fontSize: 16, fontWeight: "700" },
  cardSubtitle: { fontSize: 12, marginTop: 2 },
  dropZone: { height: 140, borderWidth: 2, borderStyle: "dashed", borderRadius: 20, justifyContent: "center", alignItems: "center", gap: 12 },
  dropZoneTitle: { fontSize: 16, fontWeight: "600" },
  inputContainer: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderRadius: 14, height: 56 },
  input: { flex: 1, height: "100%", paddingHorizontal: 12, fontSize: 16, fontWeight: "600" },
  importButton: { height: 56, borderRadius: 16, justifyContent: "center", alignItems: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4, marginTop: 12 },
