...
function ChartRenderer({ bills, activeChart, theme, t }: { bills: any[], activeChart: ChartType, theme: Theme, t: any }) {
    ...
    if (activeChart === 'creditor_pie' || activeChart === 'recurrence_pie') {
        const { data, total } = activeChart === 'creditor_pie' ? groupDataByCreditor(bills, theme) : groupDataByRecurrence(bills, theme);
        ...
        return (
            <View style={styles.chartInnerContainer}>
                <Text style={[styles.chartTotal, { color: theme.colors.primaryText }]}>{t('Total Bills Amount')}: ${centsToDollars(total)}</Text>
                <PieChart data={data} donut radius={chartWidth * 0.35} innerRadius={chartWidth * 0.25} centerLabelComponent={() => (<View style={{ justifyContent: 'center', alignItems: 'center' }}><Text style={{ fontSize: 14, color: theme.colors.subtext, fontWeight: '600' }}>{activeChart === 'creditor_pie' ? t('Creditors') : t('Recurrence')}</Text></View>)} />
