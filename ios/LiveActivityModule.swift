...
class LiveActivityModule: RCTEventEmitter {
  ...
  func clearAllSavedBills() {
    ...
    // MARK: - Consume pending paid id (mailbox)
  ...
  func consumeLastPaidBillId(_ resolve: RCTPromiseResolveBlock, _ reject: RCTPromiseRejectBlock) {
    let prefs = UserDefaults(suiteName: BillDataStore.suiteName)
    let id = prefs?.string(forKey: lastPaidBillKey)
    
    if id != nil {
      prefs?.removeObject(forKey: lastPaidBillKey)
      prefs?.synchronize()
    }
    ...
    do {
      ...
      let bills: [Bill] = arr.compactMap { (dict) -> Bill? in
        ...
        let notes = dict["notes"] as? String ?? ""
          ...
          // Support either key style from JS:
        let paymentMethod =
        (dict["payment_method"] as? String)
        ?? (dict["paymentMethod"] as? String)
        ?? ""
      ...
      let prefs = UserDefaults(suiteName: BillDataStore.suiteName)
      let bytes = prefs?.data(forKey: "saved_bills")?.count ?? -1
      print("APP DEBUG: saved bills count=\(bills.count), bytes=\(bytes)")
